...
function proseshitung()
{
	$.ajax({
		...
		error:function(){
			$("#respon").html('Proses hitung seleksi Client gagal');
			$("#error").show();
		},
		beforeSend:function(){
			$("#error").hide();
...
<?php
	...
	if(!empty($dAlternatif))
	{
		...
		foreach($dAlternatif as $rAlternatif)
		{
			...
			$clientID=$rAlternatif->id_client;
			$nama_client=field_value('client','id_client',$clientID,'nama_client');
